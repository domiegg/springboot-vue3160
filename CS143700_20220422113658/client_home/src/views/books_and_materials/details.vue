<template>
	<div class="diy_details page_books_and_materials" id="books_and_materials_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_books_and_materials'>
							<div_books_and_materials :obj="obj"></div_books_and_materials>
						</div>
					</div>
				</div>
			</div>
...
<script>
	import div_books_and_materials from "@/components/diy/div_books_and_materials.vue";
	...
	export default {
		...
		data(){
			return{
				...
				form_comment: {
					...
					source_field: "books_and_materials_id",
					source_id: 0,
					reply_to_id: 0,
				},
				count_pages: 1, // 页面数
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/books_and_materials/set?books_and_materials_id=' + obj["books_and_materials_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							source_table: "books_and_materials",
							source_field: "books_and_materials_id",
							source_id: this.obj.books_and_materials_id,
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.books_and_materials_id;
				}
			},
			/**
			 * 初始化收藏状态
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "books_and_materials",
					source_field: "books_and_materials_id",
					source_id: obj["books_and_materials_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/books_and_materials/set?books_and_materials_id=' + _this.obj["books_and_materials_id"], {
								praise_len
							}, (res) => {
								if(res.result){
