<template>
	<div class="diy_details page_classic_literature" id="classic_literature_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_classic_literature'>
							<div_classic_literature :obj="obj"></div_classic_literature>
						</div>
					</div>
				</div>
			</div>
...
<script>
	import div_classic_literature from "@/components/diy/div_classic_literature.vue";
	...
	export default {
		...
		data(){
			return{
				...
				form_comment: {
					...
					source_field: "classic_literature_id",
					source_id: 0,
					reply_to_id: 0,
				},
				count_pages: 1, // 页面数
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/classic_literature/set?classic_literature_id=' + obj["classic_literature_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							source_table: "classic_literature",
							source_field: "classic_literature_id",
							source_id: this.obj.classic_literature_id,
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.classic_literature_id;
				}
			},
			/**
			 * 初始化收藏状态
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "classic_literature",
					source_field: "classic_literature_id",
					source_id: obj["classic_literature_id"],
					user_id
				};
