<template>
	<div class="diy_details page_video_data" id="video_data_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_video_data'>
							<div_video_data :obj="obj"></div_video_data>
						</div>
					</div>
				</div>
			</div>
...
<script>
	import div_video_data from "@/components/diy/div_video_data.vue";
	...
	export default {
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/video_data/set?video_data_id=' + obj["video_data_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							source_table: "video_data",
							source_field: "video_data_id",
							source_id: this.obj.video_data_id,
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.video_data_id;
				}
			},
			/**
			 * 初始化收藏状态
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/video_data/set?video_data_id=' + _this.obj["video_data_id"], {
								praise_len
							}, (res) => {
								if(res.result){
