		        ...
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/development_history/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_development_history = !show_list_development_history">
		                        <div class="switch_box">
		                            <span v-if="show_list_development_history"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!--发展历程推荐列表组件 -->
		                <list_development_history v-if="show_list_development_history" :list="list_development_history" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_development_history.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_development_history" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_development_history" :key="i" @click="to_details('development_history',o,'development_history_id')">
		                            <td v-for="(oj,n) in fields_development_history">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
		        ...
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/video_data/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_video_data = !show_list_video_data">
		                        <div class="switch_box">
		                            <span v-if="show_list_video_data"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!--视频资料推荐列表组件 -->
		                <list_video_data v-if="show_list_video_data" :list="list_video_data" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_video_data.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_video_data" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_video_data" :key="i" @click="to_details('video_data',o,'video_data_id')">
		                            <td v-for="(oj,n) in fields_video_data">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
										<span v-else-if="oj.type && oj.type == 'UID' ">
											<span style="display: none">
...
<script>
	...
	import list_development_history from "@/components/diy/list_development_history.vue";
	import list_video_data from "@/components/diy/list_video_data.vue";
	...
	export default {
		...
		data() {
			return {
				...
				show_list_development_history: true,
				list_development_history: [],
				vm_arr_development_history: [
				...
				fields_development_history: [
					...
					{
						key: "development_significance",
						label: "发展意义",
						type: "多文本"
					},
				],
				show_list_video_data: true,
				list_video_data: [],
				vm_arr_video_data: [
					"video_data_id",
					"video_name",
		...
		methods: {
			...
			get_development_history() {
				let url = "~/api/development_history/get_list?";
				this.$get(url, {
					"page": 1,
					"size": 8
				}, (json) => {
					if (json.result) {
						this.list_development_history = json.result.list;
					}
				})
			},
			// 获取视频资料列表
			get_video_data() {
				let url = "~/api/video_data/get_list?";
				this.$get(url, {
					"page": 1,
					"size": 8
				}, (json) => {
					if (json.result) {
						this.list_video_data = json.result.list;
					}
				})
			},
		...
		computed: {
		    list_table_development_history() {
		        var list = this.list_development_history;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_development_history.map((o) => {
		                // 第二个中括号是对象的属性
		    ...
		    list_table_video_data() {
		        var list = this.list_video_data;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_video_data.map((o) => {
		                // 第二个中括号是对象的属性
		                list_table[i][o] = list[i][o] || "";
		            });
		        }
