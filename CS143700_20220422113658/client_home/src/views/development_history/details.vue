<template>
	<div class="diy_details page_development_history" id="development_history_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_development_history'>
							<div_development_history :obj="obj"></div_development_history>
						</div>
					</div>
				</div>
			</div>
...
<script>
	import div_development_history from "@/components/diy/div_development_history.vue";
	...
	export default {
		...
		data(){
			return{
				...
				form_comment: {
					...
					source_field: "development_history_id",
					source_id: 0,
					reply_to_id: 0,
				},
				count_pages: 1, // 页面数
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/development_history/set?development_history_id=' + obj["development_history_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							source_table: "development_history",
							source_field: "development_history_id",
							source_id: this.obj.development_history_id,
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.development_history_id;
				}
			},
			/**
			 * 初始化收藏状态
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "development_history",
					source_field: "development_history_id",
					source_id: obj["development_history_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/development_history/set?development_history_id=' + _this.obj["development_history_id"], {
								praise_len
							}, (res) => {
								if(res.result){
